...
import de.u808.simpleinquest.config.IndexerConfigurationBean;
...
public class DefaultIndexerFactory implements IndexerFactory, InitializingBean {
    ...
    private void initIndexerMap() {
        Map<String, String> mimeTypeIndexerMap = this.indexerConfigurationBean.getIndexerConfiguration()
                .getMimeTypeIndexerMap();
        // Map<String, String> mimeTypeIndexerMap =
        // SystemConfig.getInstance().getConfiguration().getIndexerConfiguration().getMimeTypeIndexerMap();
        ...
        for (String typeKey : mimeTypeIndexerMap.keySet()) {
            ...
            try {
                ...
                SystemConfig.getInstance().addError(
                        new ConfigurationError("Indexer class " + indexerClassName + " not found in classpath.",
                                ConfigurationError.Severity.WARNING, e));
            } catch (SecurityException e) {
                SystemConfig.getInstance().addError(
                        new ConfigurationError("SecurityException while instantiate class " + indexerClassName,
                                ConfigurationError.Severity.WARNING, e));
            } catch (NoSuchMethodException e) {
                SystemConfig.getInstance().addError(
                        new ConfigurationError("NoSuchMethodException while instantiate class " + indexerClassName,
                                ConfigurationError.Severity.WARNING, e));
            } catch (IllegalArgumentException e) {
                SystemConfig.getInstance().addError(
                        new ConfigurationError("IllegalArgumentException while instantiate class " + indexerClassName,
                                ConfigurationError.Severity.WARNING, e));
            } catch (InstantiationException e) {
                SystemConfig.getInstance().addError(
                        new ConfigurationError("InstantiationException while instantiate class " + indexerClassName,
                                ConfigurationError.Severity.WARNING, e));
            } catch (IllegalAccessException e) {
                SystemConfig.getInstance().addError(
                        new ConfigurationError("IllegalAccessException while instantiate class " + indexerClassName,
                                ConfigurationError.Severity.WARNING, e));
            } catch (InvocationTargetException e) {
                SystemConfig.getInstance().addError(
                        new ConfigurationError("InvocationTargetException while instantiate class " + indexerClassName,
                                ConfigurationError.Severity.WARNING, e));
            }
